...
const ListingDetailScreen = ({ route, navigation }) => {
  ...
  const getPriceType = (priceType) => {
    ...
    if (!!pU && listingData?.price_units?.length) {
      const tempUnits = listingData.price_units.filter((_pu) => _pu.id === pU);
      const tempUnit = tempUnits.length ? tempUnits[0].name : null;
      if (priceType === "fixed") {
        return tempUnit
  ...
  const getListingTime = () => {
    if (listingData?.date_created) {
      return moment
        .parseZone(
          listingData.date_created.date + listingData.date_created.timezone
        )
        .fromNow();
    } else {
